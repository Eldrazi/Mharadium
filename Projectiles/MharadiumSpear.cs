...
{
    public class MharadiumSpear : ModProjectile
    {
        public override void SetDefaults()
        {
            projectile.name = "Mharadium Spear";
            projectile.width = 18;
        ...
        public override void AI()
        {
            ...
            {
                int num260 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 57, projectile.velocity.X * 0.2f, projectile.velocity.Y * 0.2f, 200, default(Color), 1.2f);
                Main.dust[num260].velocity += projectile.velocity * 0.3f;
                Main.dust[num260].velocity *= 0.2f;
            }
            if (Main.rand.Next(4) == 0)
            {
                int num261 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 43, 0f, 0f, 254, default(Color), 0.3f);
